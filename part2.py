...
class Bus:
    ...
    def request(self, txn: BusTxn, start_time: int) -> Tuple[BusResp, int, int]:
        ...
        for cid, snp in enumerate(self.snoopers):
            if cid == txn.src_core:
                continue
            had, supplied, invalidated, prev = snp.on_snoop(
...
def main():
    ap = argparse.ArgumentParser(
        description="Trace-driven cache coherence simulator (MESI)")
    ap.add_argument("protocol", choices=[
                    "MESI", "Dragon"], help="Coherence protocol (Dragon not implemented; use MESI).")
    ap.add_argument(
